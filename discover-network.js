...
export async function main(ns) {
	...
	ns.print(".hackerScript: " + hackerScript);
	ns.print(".mode.       : " + fullmode);
	if (fullmode == "FULL") {
		hackFullServer = true;
	}

	scriptRAM = ns.getScriptRam(hackerScript); // GB
	ns.print(".RAM per script: " + scriptRAM);
	// loop until all servers are hacked 
	// we may wait for new program to be available at relaunch time 
	areAllServerHacked = false; // start by considering we can hack them all
	while (areAllServerHacked == false) {
		...
		for (const server of servers) {
			...
			//ns.tprint(`check host ${hostname} . RAM: ${max}GB`);
			serverHacked = await calcHack(ns, server, hackFullServer, hackerScript, target);
			if (serverHacked == false) { // fail to gain root access
				areAllServerHacked = false; // not al servers hacked
			}
			hcount++;
		}
		ns.print(servers.length + " servers scanned.");
